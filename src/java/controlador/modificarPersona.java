 ...
 * @author andre
 */
@WebServlet(urlPatterns = {"/modificarPersona"})
public class modificarPersona extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        long documento = Long.parseLong(request.getParameter("id"));
        ...
        long celular = Long.parseLong(request.getParameter("cel"));
        String contra = request.getParameter("pass");
        persona per = new persona();
        try (PrintWriter out = response.getWriter()) {
            if (per.existe(documento, tipo)) {
                if (per.modificar(documento, tipo, nombre, apellido, correo, celular, contra)) {
